...
class NestedNewtonSolver(dolfinx.cpp.nls.petsc.NewtonSolver):
    def __init__(
        ...
        #print("intialized %s", repr(self._b.getSize()))
        self._J = dolfinx.fem.petsc.create_matrix_nest(self._a)
        #print("intialized jacobian %s %s", repr(self._J.getSize()), repr(self._J.getType()))
    ...
    def _pre_newton_iteration(self, x: PETSc.Vec) -> None:
        ...
        """
        #print("preiteration %s", repr(x.getArray()))
        ...
        #print("Post-preiteration %s",repr(x.getArray()))
    ...
    def _assemble_residual(self, x: PETSc.Vec, b: PETSc.Vec) -> None:
        ...
        #print("Before Assembling %s %s %s %s", repr(b.getType()),repr(b.getArray()),repr(x.getType()),repr(x.getArray()))
        ...
        #print("After assembly residual %s %s", repr(b.getArray()),repr(x.getArray()))
        dolfinx.fem.petsc.apply_lifting_nest(b, self._a, self._bcs)
        #print("After lifitng residual %s %s", repr(b.getArray()),repr(x.getArray()))
        for b_sub in b.getNestSubVecs():
        ...
        #print("After Assembling %s %s %s %s", repr(b.getType()),repr(b.getArray()),repr(x.getType()),repr(x.getArray()))
    ...
    def _assemble_jacobian(self, x: PETSc.Vec, A: PETSc.Mat) -> None:
        ...
        """
        # Assemble Jacobian
        #print("Jacobian type is: %s", repr(A.getType()))
        A.zeroEntries()
        dolfinx.fem.petsc.assemble_matrix_nest(A, self._a, bcs=self._bcs)
        A.assemble()
        #print("Jacobian type is: %s", repr(A.getType()))
        #print("Size matrix %s",repr(A.getSize()))
    ...
    def _update_function(self, solver, dx: PETSc.Vec, x: PETSc.Vec):
        #print("started updating %s %s ", repr(self._u[0].x.array),repr(self._u[1].x.array))
        #print("dx= %s %s",repr(dx.getType()),repr(dx.getArray()))
        ...
        #     ui.x.petsc_vec.ghostUpdate(addv=PETSc.InsertMode.INSERT, mode=PETSc.ScatterMode.FORWARD)
        #     offset_start += num_sub_dofs
        #print("DONE updating %s %s ", repr(self._u[0].x.array),repr(self._u[1].x.array))
...
class NestedNewtonSolverMPC(dolfinx.cpp.nls.petsc.NewtonSolver):
    def __init__(
        self,
        F: list[ufl.form.Form],
        u: list[dolfinx.fem.Function],
        ...
        self._bcs = bcs
        self._u = u
        self.u_mpc =  [dolfinx.fem.Function(m.function_space) for m in self._mpc]
        self._pre_solve_callback: Callable[["NestedNewtonSolver"], None] | None = None
        self._post_solve_callback: Callable[["NestedNewtonSolver"], None] | None = None
        ...
        #print("intialized jacobian %s %s", repr(self._J.getSize()), repr(self._J.getType()))
        self._dx = dolfinx_mpc.create_vector_nest(self._F, self._mpc)
        self._x = dolfinx_mpc.create_vector_nest(self._F, self._mpc)
        self._J.setOptionsPrefix(prefix)
        self._J.setFromOptions()

        self.setF(self._assemble_residual, self._b) # done
        self.setJ(self._assemble_jacobian, self._J)
        self.set_form(self._pre_newton_iteration)
        self.set_update(self._update_function)
    ...
    def _assemble_residual(self, x: PETSc.Vec, b: PETSc.Vec) -> None:
        ...
        # with b.localForm() as b_local:
        #     b_local.set(0.0)
        dolfinx_mpc.assemble_vector_nest(
            b,
            self._F,
            self._mpc
        )
        
        #print("After assembling residual %s %s", repr(b.getArray()),repr(x.getArray()))
        dolfinx.fem.petsc.apply_lifting_nest(b, self._a, self._bcs)
        #print("After lifitng residual %s %s", repr(b.getArray()),repr(x.getArray()))
        for b_sub in b.getNestSubVecs():
            b_sub.ghostUpdate(addv=PETSc.InsertMode.ADD, mode=PETSc.ScatterMode.REVERSE)
    ...
    def _assemble_jacobian(self, x: PETSc.Vec, A: PETSc.Mat) -> None:
        ...
        #print("Jacobian type is: %s", repr(A.getType()))
        A.zeroEntries()
        dolfinx_mpc.assemble_matrix_nest(A, self._a, self._mpc, self._bcs)
        A.assemble()
        #print("Jacobian type is: %s", repr(A.getType()))
        #print("Size matrix %s",repr(A.getSize()))
        # #print( A.convert('seqaij').getValuesCSR())
    ...
    def _update_function(self, solver, dx: PETSc.Vec, x: PETSc.Vec):
        ...
        for i,ui in enumerate(self._u):
            self.u_mpc[i].x.petsc_vec.array = ui.x.petsc_vec.array_r
            Vi = ui.function_space
            num_sub_dofs = Vi.dofmap.index_map.size_local * Vi.dofmap.index_map_bs
            #print("replace = ",self.u_mpc[i].x.petsc_vec.array )
            self.u_mpc[i].x.petsc_vec.array_w[:num_sub_dofs] -= (
                self.relaxation_parameter * dx.array_r[offset_start : offset_start + num_sub_dofs]
            )
            #print("Correct = ",self.u_mpc[i].x.petsc_vec.array )
            self.u_mpc[i].x.petsc_vec.ghostUpdate(addv=PETSc.InsertMode.INSERT,mode=PETSc.ScatterMode.FORWARD)
            
            self._mpc[i].homogenize(self.u_mpc[i])
            #print("Homo = ",self.u_mpc[i].x.petsc_vec.array )
            
            self._mpc[i].backsubstitution(self.u_mpc[i])
            #print("backs = ",self.u_mpc[i].x.petsc_vec.array )
            
            ui.x.array[:] = self.u_mpc[i].x.petsc_vec.array_r
            ui.x.petsc_vec.ghostUpdate(addv=PETSc.InsertMode.INSERT, mode=PETSc.ScatterMode.FORWARD)
